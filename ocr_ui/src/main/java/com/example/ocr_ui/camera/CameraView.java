...
import com.example.ocr_ui.util.DimensionUtil;
import com.example.ocr_ui.util.ImageUtil;
...
//        }
    ...
    private Bitmap crop(File outputFile, byte[] data, int rotation) {
        Log.e("拍摄后的照片", "crop: " + outputFile.getAbsolutePath() + " , " + data + " , " + rotation);
        //  E/拍摄后的照片: crop: /data/user/0/com.jxaic.wsdj/files/idCard/front/pic.jpg , [B@3b36f2a , 90
        try {
            Rect previewFrame = cameraControl.getPreviewFrame();
            if (maskView.getWidth() == 0 || maskView.getHeight() == 0
                    || previewFrame.width() == 0 || previewFrame.height() == 0) {
